...
class WalletController extends Controller
{
    ...
    public function Subscribe(Request $request)
    {
        ...
        $hasAlready = Subscriberwhere('payment_data',$request->payment_data)->first();
        if($hasAlready){
            $lastuser =User::where('id',$hasAlready->user_id)->first();
            $user = Auth::user();
            if($user->email == $lastuser->email){
                return response()->json(['status' => true, 'data' => 'Successfully  subscribed  plan. ','debug'=>'restored true']);
            } else {
                return response()->json(['status' => false, 'data' => 'No purchase Found ','debug'=>'restored false']);
            }
    ...
    public function Withdraw(Request $request)
    {
        $has = Withdraw::where('user_id',Auth::id())
            ...
            ->get()->count();
        if($has):
            $lastWithdraw = Withdraw::where('user_id',Auth::id())->latest()->first();
            $start = (new Carbon($lastWithdraw->created_at))->addDays(30);
            $end = Carbon::today();
            $days = $start->diff($end)->days;

            return response()->json(['status'=>false,'data'=>'Withdraw available after '.$days.' days','debug'=>'try after 1 minutes for testing ']);
        endif;
        $hasSubscription = User::where('id',Auth::id())->with('Subscription')->first();
        //return response()->json(['status'=>false,'data'=>$hasSubscription]);
        if(!$hasSubscription):
            return response()->json(['status'=>false,'data'=>'Redeem only available on Paid Wallet', 'line'=> __LINE__]);
        endif;
