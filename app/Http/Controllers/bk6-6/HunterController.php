...
class HunterController extends Controller
{
	...
	public function CatchIfYouCan(Request $request)
    {
        ...
        Log::info('Usercame '.$user->email.' lat='.  $lat.' lng='.$lng.' distance='.$distance.' has='.$has);
        ...
        //var_dump(count($Rest));
        Log::info('Usercamegot '.$user->email.' lat='.  $lat.' lng='.$lng.' distance='.$distance.' has='.$has.' total'.$total.' sentids '. implode(',',$sent));
        return response()->json([
            'total'=>$total,
            'coinArroundMe'=>$Rest,
            'status'=>true,
    ...
    //     die;
            ...
            if($targetCoin->used){
                if($targetCoin->user_id){
                    ...
                    if($grabber){
                        ...
                        {
                            Log::info('caching error-same triedby '.$user->email.' lat='.  $lat.' lng='.$lng.'  coinid='. $request->id .' line='. __LINE__);
                            return response()->json([
                                'status'=>false,
                                'caught'=>false,
                                'upgrade'=>false,
                        ...
                        {
                            Log::info('caching error-othername='.$grabber->username.' triedby '.$user->email.' lat='.  $lat.' lng='.$lng.'  coinid='. $request->id.' line='. __LINE__);
                        ...
                        Log::info('caching error-SomeoneElseGrabbed triedby '.$user->email.' lat='.  $lat.' lng='.$lng.'   coinid='. $request->id.' line='. __LINE__);
        ...
        if(sizeof($coinArroundMe)==0){
            Log::info('caching Coin not within your radius'.$user->email.' lat='.  $lat.' lng='.$lng.'  coinid='. $request->id.' line='. __LINE__);
        ...
        $user= auth()->user();
        if(!$user){
            Log::info('caching please re login'.$user->email.' lat='.  $lat.' lng='.$lng.'  coinid='. $request->id.' line='. __LINE__);
        ...
        Log::info('grabbed'.$user->email.' lat='.  $lat.' lng='.$lng.'   coinid='. $request->id.' line='. __LINE__);
        return response()->json([
            'status'=>true,
            'bal'=>$bal,
            ...
            if($coin->py_name == 'BTC' || $coin->py_name == 'LTC' || $coin->py_name == 'ETH'){
                $url='https://api.coinbase.com/v2/prices/'.$coin->py_name.'-USD/spot';
                        ...
                        if(is_array($mn_data)){
                            ...
                            Coins::where('id',$id)->update(['price'=>$p, 'fraction'=>$cid]);
                            //echo 'updating';
                            ...
                            if (is_array($mn_data)){
                                if (sizeof($mn_data) > 0){
                                    ...
                                    if(array_key_exists('price_usd',$found)){
                                        ...
                                        Coins::where('id',$id)->update(['price'=>$found['price_usd'], 'fraction'=>$cid]);
                                        //echo 'updating';
                                        if(!Cache::has('usd_price'.$coin->id)){
    ...
    public function IsNewLocation($lat,$lng){
        ...
        if($IsNewLocation <1){
            ...
            Log::info('old user on new location added coin = '.$added);
        }
